...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/finbridge/manage/fbAssetInfo/">挂牌资产信息表列表</a></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="fbAssetInfo">
			<tr>
				...
				<td>${fns:getFbDictLabel(fbAssetInfo.isreCommend,'yesOrNo',' ')}</td>
				<td>${fns:getFbDictLabel(fbAssetInfo.listStatus,'listStatus',' ')}</td>
				<td>${fns:getFbDictLabel(fbAssetInfo.aduitStatus,'aduitStatus',' ')}</td>
				<shiro:hasPermission name="finbridge:manage:fbAssetInfo:edit"><td>
    				<a href="${ctx}/finbridge/manage/fbAssetInfo/formFinish?id=${fbAssetInfo.id}">查看</a>
					<a href="${ctx}/finbridge/manage/fbAssetInfo/recommend?id=${fbAssetInfo.id}" onclick="return confirmx('确认要推荐该资产信息表吗？', this.href)">推荐</a>
					<a href="${ctx}/finbridge/manage/fbAssetInfo/unRecommend?id=${fbAssetInfo.id}" onclick="return confirmx('确认要取消推荐该资产信息表吗？', this.href)">取消</a>
